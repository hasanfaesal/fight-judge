"""
Fix and normalize COCO-format keypoint annotation JSON files.

This script is used after running batch pose inference (run-inference-batch.py).
The raw inference output contains bounding boxes in [x1, y1, x2, y2] format,
but COCO standard requires [x, y, width, height]. This script:
  - Converts bbox format from corner coordinates to top-left + dimensions
  - Adds required COCO metadata fields (area, iscrowd, num_keypoints)
  - Injects a compliant categories block with 17-point COCO keypoint schema
  - Adds info and licenses sections
  - Creates backups before modifying files in-place

Input:  annotations/pose_results_{train,valid,test}.json
Output: Same files overwritten in-place (originals backed up to annotations_backup/)
