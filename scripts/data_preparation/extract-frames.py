...
This is the first step in the data collection pipeline. Fight footage (MP4/MOV)
is decomposed into individual frames that are then uploaded to Roboflow for
manual bounding box annotation of the 'fighter' class.

The script is interactive: it prompts the user for a video path and an output
folder at runtime. It logs video metadata (resolution, FPS, duration, codec)
and writes a timestamped .log file alongside the extracted frames.

Output naming: frame_000000.jpg, frame_000001.jpg, ...

Usage:
    python extract-frames.py
    # Then enter video path and output folder when prompted.
"""
...
def setup_logging(output_folder):
    ...
    log_filename = os.path.join(
        output_folder, f"extraction_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    )
...
def extract_video_frames(video_path, output_folder="extracted_frames"):
    ...
    while True:
        ...
        if (frame_number + 1) % 100 == 0 or (frame_number + 1) == frame_count:
            ...
            logger.info(
                f"Progress: {frame_number + 1}/{frame_count} frames ({progress:.1f}%)"
            )
    ...
    logger.info(
        f"Extraction time: {extraction_duration:.2f} seconds ({extraction_duration / 60:.2f} minutes)"
    )
    logger.info(
        f"Average speed: {extracted_count / extraction_duration:.2f} frames/second"
    )
...
if __name__ == "__main__":
    ...
    custom_output = input(
        "Enter output folder name (press Enter for 'extracted_frames'): "
    ).strip()
